...
namespace Sfs2XExamples.SpaceWar
{
	public class Main : MonoBehaviour 
	{
		...
		void DoConnect()
		{
			...
			if (Application.isWebPlayer || Application.isEditor) {
				if (!Security.PrefetchSocketPolicy(serverName, serverPort, 5000)) {
					Debug.Log("Security Exception. Policy file loading failed!");
				}
			}	

			sfs.Connect(serverName, serverPort);
		...
		void DisposeSFS()
		{
			// Remove SFS2X event listenerssfs.AddEventListener(SFSEvent.CONNECTION, OnConnection);
		...
		public void OnConnection(BaseEvent evt) 
		{
			...
			{
				loginStep = 1;
				errorMsg = "Unable to connect to " + serverName + ":" + serverPort + "\nIs the server running at all?";
		...
		{
			User user = evt.Params["user"] as User;
			//ArrayList changedVars = evt.Params["changedVars"] as ArrayList; 
		...
		// This events are used so that the communication with SmartFoxServer
